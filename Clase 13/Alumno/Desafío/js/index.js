...
function validateEmail(event) {
  ...
  if (!value) {
		...
		if($('#rq-email').length === 0){
			inputNode.parent().append('<div id="rq-email" style="color: red">*El campo es requerido</div>')
		} 
    	...
    	if($('#rq-at').length === 0){
			inputNode.parent().append('<div id="rq-at" style="color: red">*falta un arroba</div>')
		}
    	...
    	if($('#rq-point').length === 0){
			inputNode.parent().append('<div id="rq-point" style="color: red">*falta un punto</div>')
		}
...
function validateComments(event) {
	...
	if(!valueInputComments){
		...
		inputCommentsNode.parent().append('<div id="rq-comments" style="color: red">*El campo es requerido</div>')
		$('#min-length').remove()
		...
		if($('#min-length').length === 0){
			inputCommentsNode.parent().append('<div id="min-length" style="color: red">*minimo de letras 8</div>')
		} 
...
function validateName (event) {
    ...
    if (!value) {
      ...
      inputNode.addClass('is-invalid')
		...
		if($('#rq-name').length === 0){
			inputNode.parent().append('<div id="rq-name" style="color: red">*El campo es requerido</div>')
		} 
